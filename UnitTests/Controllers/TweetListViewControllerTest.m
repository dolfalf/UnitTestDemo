...
- (void)setUp {
	...
	controller = [[TweetListViewController alloc] initWithTwitterUser:user];
...
- (void)testInitWithUser {
	// Required init code already invoked in setUp method, simply validate its state
	GHAssertEquals(controller.tableView.style, UITableViewStylePlain, nil);
	GHAssertEquals(controller.user, user, nil);
	GHAssertNotNil(controller.twitterService, nil);
}
...
- (void)testViewWillAppear_withStubService {
	...
	id stubService = [[[StubTwitterService alloc] initWithTweets:tweets] autorelease];
...
- (UITableViewCell *)invokeCodeForCell:(UITableViewCell *)expectedCell {
	...
	[[[mockTableView expect] andReturn:expectedCell] dequeueReusableCellWithIdentifier:@"twitterCell"];
...
- (void)testTableViewCellForRowAtIndexPath_withNilCell {
	UITableViewCell *resultCell = [self invokeCodeForCell:nil];
	[self invokeAssertsForResultCell:resultCell];
}
